...
async function getSpreadsheet() {
  try {
    ...
    const jwt = new google.auth.JWT(process.env.GOOGLE_SHEETS_CLIENT_EMAIL, null, process.env.GOOGLE_SHEETS_PRIVATE_KEY, target)
    ...
    function rowToEvent(row) {
      let EXPECTED_COLUMNS = ["Date", "End Date", "Day of Week", "Start Time", "End Time", "Name", "Description", "Location", "Division", "Collaborator(s)", "Public"]
      function getValue(column) { return row[EXPECTED_COLUMNS.indexOf(column)] }
...
export async function getStaticProps() {
  ...
  let data
  await client
    .fetch('*[_type == "eventspage"]{who, where, "images": images[].asset->url}')
  ...
  return {
    props: { events, data, registeredTeams }
  }
}
