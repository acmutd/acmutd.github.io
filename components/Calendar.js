  ...
  render() {
    return (
      <div
        ...
        style={{
          '--color': this.props.color || '#00ffff77',
...
export class Day extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <div className="px-1 py-0 w-min font-black text-lg">
          {this.props.date}
        </div>
        <div className="events flex flex-col h-calendar text-base overflow-auto overflow-x-hidden rounded">
          {events}
        </div>
      </div>
    );
...
export default class Calendar extends Component {
  ...
  render() {
    const svg = (c, f) => (
      <svg className={c} onClick={f} viewBox="0 0 448 512">
        <path
          fill="currentColor"
    ...
    return (
      <div className="w-4/5 m-auto">
        ...
        <div className="flex justify-center items-center w-full">
          {svg(
            'transition transform -rotate-90 my-0 mx-1 w-6 h-6 cursor-pointer hover:text-gray-500',
            () =>
              this.setState({
        ...
        <div className="flex w-full m-auto text-puny font-bold lg:text-base">
          {weekdays}
        </div>
        <div className="flex flex-wrap p-0">{days}</div>
      </div>
    );
  }
}
